...
import io.agora.hack.believe.databinding.ViewCommonTitleBinding
...
class CommonTitleView : ConstraintLayout {
    ...
    private var binding: ViewCommonTitleBinding
    ...
    constructor(context: Context, attrs: AttributeSet?, defStyleAttr: Int, defStyleRes: Int) : super(
        ...
        binding = ViewCommonTitleBinding.inflate(LayoutInflater.from(context), this)
        initView(context, attrs, defStyleAttr)
    }

    private fun initView(context: Context, attrs: AttributeSet?, defStyleAttr: Int) {
        val ta = context.theme.obtainStyledAttributes(attrs, R.styleable.title_view, defStyleAttr, 0)
        val leftImageDrawable: Drawable? = ta.getDrawable(R.styleable.title_view_ctv_leftImageSrc)
        leftImageDrawable?.let {
            binding.ivBackIcon.setImageDrawable(it)
            binding.ivBackIcon.isVisible = true
        }
        val isHideLeftImage: Boolean = ta.getBoolean(R.styleable.title_view_ctv_hideLeftImage, false)
        binding.ivBackIcon.isGone = isHideLeftImage

        val leftText = ta.getString(R.styleable.title_view_ctv_leftText)
        ...
        val centerText = ta.getString(R.styleable.title_view_ctv_centerText)
        ...
        val centerTextColor = ta.getColor(R.styleable.title_view_ctv_centerTextColor, Color.BLACK)
        binding.tvCenterTitle.setTextColor(centerTextColor)

        val rightImageDrawable: Drawable? = ta.getDrawable(R.styleable.title_view_ctv_rightImageSrc)
        rightImageDrawable?.let {
            binding.ivRightIcon.setImageDrawable(it)
            binding.ivRightIcon.isVisible = true
        }
        val isHideRightImage: Boolean = ta.getBoolean(R.styleable.title_view_ctv_hideRightImage, true)
        binding.ivRightIcon.isGone = isHideRightImage

        val rightText = ta.getString(R.styleable.title_view_ctv_rightText)
