    ...
    dispatch_source_set_event_handler(self.timer, ^{
        if(timeout <= 0){ //倒计时结束，关闭
            ...
            dispatch_async(dispatch_get_main_queue(), ^{
                ...
                [sender setTitle:MCLocalizedString(@"发送验证码") forState:UIControlStateNormal];
                //设置不可点击
            ...
            dispatch_async(dispatch_get_main_queue(), ^{
                //设置界面的按钮显示 根据自己需求设置
                [sender setTitle:[NSString stringWithFormat:MCLocalizedString(@"%@秒后可重新发送"), strTime] forState:UIControlStateNormal];
                    //设置可点击
...
- (UILabel *)titleLabel {
    if (!_titleLabel) {
        ...
        _titleLabel.text = MCLocalizedString(@"验证码");
        _titleLabel.textColor = [[UIColor alloc] initWithHexString:@"#979CBB"];
...
- (UITextField *)vTextField {
    if (!_vTextField) {
        ...
        _vTextField.attributedPlaceholder = [[NSAttributedString alloc] initWithString:MCLocalizedString(@"请输入验验证码") attributes:@{NSFontAttributeName: [UIFont systemFontOfSize:15],NSForegroundColorAttributeName:[[UIColor alloc] initWithHexString:@"#979CBB"]}];
        _vTextField.keyboardType = UIKeyboardTypeNumberPad;
        _vTextField.textColor = [[UIColor alloc] initWithHexString:@"#3C4267"];
...
- (UIButton *)vSendbutton {
    if (!_vSendbutton) {
        ...
        [_vSendbutton setTitle:MCLocalizedString(@"发送验证码") forState:UIControlStateNormal];
        [_vSendbutton setTitleColor:[[UIColor alloc] initWithHexString:@"#009FFF"] forState:UIControlStateNormal];
