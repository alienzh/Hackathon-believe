...
//
    ...
    func createEngine(){
        ...
        config.appId = KeyCenter.RTCAPPID
...
//        option.audienceLatencyLevel = .lowLatency
        ...
        let channelName = self.roomId ?? "111"
        let uid = UInt(AppContext.currentUserId ?? KeyCenter.RTM_UID)!
        print("uid == \(uid)")
        let ret = agoraKit.joinChannel(byToken: KeyCenter.RTCTOKEN, channelId: channelName, uid: uid, mediaOptions: option)
        print("ret == \(ret)")
...
extension WelComeViewController: AgoraRtcEngineDelegate {
